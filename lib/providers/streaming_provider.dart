...
class StreamingSessionsNotifier extends Notifier<StreamingSessionsState> {
  ...
  Future<StreamingSession?> startStreaming({
    ...
    _activeSubscription?.cancel();
    _activeSubscription = streamingService.getSessionStream(session.id)?.listen((updatedSession) {
      final newSessions = Map<String, StreamingSession>.from(state.sessions);
...
final isStreamingTorrentProvider = Provider.family<bool, String>((ref, infoHash) {
  final state = ref.watch(streamingSessionsProvider);
  return state.sessions.values.any((s) => 
...
final sortedStreamsForStreamingProvider = Provider.family<List<TorrentioStream>, List<TorrentioStream>>((ref, streams) {
  return streams.sortForStreaming();
});

/// Helper provider to get the best stream for streaming
final bestStreamForStreamingProvider = Provider.family<TorrentioStream?, List<TorrentioStream>>((ref, streams) {
